...
Images_RelPath_File.write('Image_Nom,Photo_CheminAccès,Surface_Nom,Photo_Date-Prise' '\n')
...
for dirpath, subdirs, files in os.walk(os.getcwd()) :
    ...
    for line in files :
        if line.endswith(('.jpg', '.jpeg','.tif', '.tiff')) :
            ...
            Surface_Nom = re.search('^(?:[0-9]{4})_(?:[A-Z]{3,4})_(?:[a-z]{2,3}[0-9]{1,2})_(?:(M[0-9]{1,3}-[A-Z]{1,2})-?[0-9]?-?[0-9]?-?[0-9]?-?[0-9]?)_(?:[a-zA-Z]{3,5}-[a-zA-Z]{4,12})_(?:[a-zA-Z]{2,3}).(?:jpg?|tif)$', line)
